...
/** Resolves a cart item to its correct internal route */
function getProductLink(productId: string | undefined, shape?: string): string | null {
...
const CartPage: React.FC = () => {
  ...
  return (
    <section className="min-h-[80vh] bg-orea-cream px-6 pt-16 pb-[160px]">
        ...
        {items.length === 0 ? (
          ...
          <>
            ...
            <div className="flex flex-col">
              {items.map((item, idx) => {
                const productLink = getProductLink(item.productId, item.shape);
                return (
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between mt-4">
                      <div className="flex items-center border border-orea-champagne/30">
                        ...
                        <button onClick={() => updateQuantity(item.id, item.quantity + 1)} disabled={item.quantity >= 5} className="w-10 h-10 flex items-center justify-center text-orea-taupe hover:text-orea-dark transition-colors disabled:opacity-30 disabled:cursor-not-allowed" aria-label="Increase quantity">
