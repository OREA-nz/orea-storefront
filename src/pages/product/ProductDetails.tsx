...
import GiftReminderModal from '../../components/shared/GiftReminderModal';
...
import { isEnquiryOnly } from '../../data/shopifyProducts';

// Product IDs that should show the diamond shape selector
const SOLITAIRE_PRODUCT_IDS = [
  'solitaire-necklace',
  'solitaire-bracelet',
  'solitaire-studs',
  'solitaire-pendant',
...
const ProductDetails: React.FC<ProductDetailsProps> = ({
  ...
  const isInquiryRequired = isEnquiryOnly(product.id) || selectedCarat === '3+ CT';

  useEffect(() => {
    if (product.variants) {
      // For metal+carat products match both option1 (metal) and option2 (normalised carat).
      // For metal-only products option2 is '' and selectedCarat is '' — condition still holds.
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      {showShapeSelector && selectedShape && (
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-body-sm font-light text-orea-taupe tracking-wide">{selectedShape}</span>
            <button
              type="button"
              onClick={() => setIsShapeInfoOpen(true)}
              className="text-orea-champagne hover:text-orea-dark transition-colors"
              aria-label="Diamond shape information"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
      ...
      {/* Carat */}
      {setSelectedCarat && selectedCarat !== undefined && product.options.carat.length > 0 && (
          ...
          <div className="grid grid-cols-3 gap-2">
            {product.options.carat.map((carat) => (
              <button
                ...
                className={`py-4 text-[11px] sm:text-[10px] font-bold uppercase tracking-[0.15em] sm:tracking-[0.3em] border transition-all ${
                  selectedCarat === carat
      ...
      <div className="flex flex-col gap-3 pt-6">
        ...
        {statusMsg && (
          <p className={`text-micro font-bold uppercase tracking-widest text-center animate-in slide-in-from-top-2 ${
            statusMsg.type === 'success' ? 'text-orea-success' : 'text-orea-error'
          }`}>
            {statusMsg.text}
          </p>
        )}
      ...
      {/* Metal info modal */}
      {isMetalInfoOpen && createPortal(
        <div
          className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-orea-dark/40 backdrop-blur-sm animate-in fade-in"
          onClick={() => setIsMetalInfoOpen(false)}
        >
          <div
            className="bg-orea-cream w-full max-w-lg p-10 md:p-12 rounded-sm relative shadow-2xl"
      ...
      {/* Shape info modal — solitaire non-ring products only */}
      {isShapeInfoOpen && createPortal(
        <div
          className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-orea-dark/40 backdrop-blur-sm animate-in fade-in"
          onClick={() => setIsShapeInfoOpen(false)}
        >
          <div
            className="bg-orea-cream w-full max-w-lg p-10 md:p-12 rounded-sm relative shadow-2xl"
            onClick={(e) => e.stopPropagation()}
