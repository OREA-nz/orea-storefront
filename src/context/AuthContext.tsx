...
/* â”€â”€ Password hashing (demo mode only) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const signIn = useCallback(async (email: string, password: string): Promise<boolean> => {
    ...
    // Demo mode â€” compare against stored SHA-256 hash
    const stored = localStorage.getItem('orea_demo_accounts');
    const accounts: Record<string, { passwordHash: string; firstName: string; lastName: string }> = stored ? JSON.parse(stored) : {};
  ...
  const signUp = useCallback(async (
    ...
    // Demo mode â€” store SHA-256 hash, never plaintext
    const stored = localStorage.getItem('orea_demo_accounts');
    const accounts: Record<string, { passwordHash: string; firstName: string; lastName: string }> = stored ? JSON.parse(stored) : {};
    if (accounts[email]) return false; // already exists
    const passwordHash = await hashPassword(password);
    accounts[email] = { passwordHash, firstName, lastName };
