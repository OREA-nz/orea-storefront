...
let nextId = 1;
const MAX_CART_QTY = 5; // Luxury items: cap per line item at 5
...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [items, setItems] = useState<CartItem[]>(() => {
    try {
      ...
      // Corrupt or tampered localStorage — reset to empty cart
  ...
  }, [shopify.isLive]); // re-run when Shopify config resolves
  ...
  const updateQuantity = useCallback((id: string, quantity: number) => {
    if (quantity <= 0) {
      ...
      setItems((prev) => prev.map((i) => (i.id === id ? { ...i, quantity: capped } : i)));
    }
  }, []);
  ...
  const redirectToCheckout = useCallback(() => {
    ...
    if (url) {
      ...
      const checkoutUrl = url.replace('orea-8820.myshopify.com', 'www.orea.co.nz');
